...
class histogram2(object):
    def __new__(cls, RESULT_FOLDER_PATH, SOURCES_FOLDER_PATH, lmdas, lista_porcentajes):
        ...
        result = plotly.offline.plot(fig, filename=RESULT_FOLDER_PATH + 'DOCODE-VvsF.html')

        text_prueba.write(" DOCODE NORMALIZADO \n")
        data2 = xlrd.open_workbook(RESULT_FOLDER_PATH + 'ResultDOCODENORM.xls')
        table2 = data2.sheets()[0]
        colors = ['rgba(0, 0, 255, 1)', 'rgba(200, 0, 0, 1)', 'rgba(0, 0, 255, 0.8)', 'rgba(200, 0, 0, 0.8)',
                  'rgba(0, 0, 255, 0.6)', 'rgba(200, 0, 0, 0.6)', 'rgba(0, 0, 255, 0.4)', 'rgba(200, 0, 0, 0.4)',
                  'rgba(0, 0, 255, 0.2)', 'rgba(200, 0, 0, 0.2)']
        ...
        fig = go.Figure(data=traces1)
        fig['layout'].update(height=1200, width=1200, title='Verdaderos vs Falsos')
        result = plotly.offline.plot(fig, filename=RESULT_FOLDER_PATH + 'DOCODE-NORM-VvsF.html')
        ...
        fig = go.Figure(data=traces1)
        fig['layout'].update(height=1200, width=1200, title='Verdaderos vs Falsos')
        result = plotly.offline.plot(fig, filename=RESULT_FOLDER_PATH + 'DOCODE-NORM-SEG-VvsF.html')